...
export default function Page() {
  ...
  async function submitContact(event: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await createClient().from('contact_requests').insert({ name: String(form.get('name')), email: String(form.get('email')), phone: String(form.get('phone') || ''), subject: String(form.get('subject')), message: String(form.get('message')) })
    if (error) { alert('Não foi possível enviar o pedido. Tente novamente.'); return }
    event.currentTarget.reset(); alert('Pedido enviado. Entraremos em contacto consigo.')
  }
  ...
  return (
    <main>
      ...
      <section id="contactos" className="section contact-section"><div className="container contact-grid"><div><p className="eyebrow eyebrow-dark">CONTACTOS</p><h2>Vamos conversar sobre a sua <em>questão jurídica.</em></h2><p>Partilhe connosco, de forma segura, o contexto da sua necessidade. A nossa equipa entrará em contacto consigo.</p><div className="contact-details"><div><Phone /><span>Telefone<strong>{officePhoneOne} · {officePhoneTwo}</strong></span></div><div><Mail /><span>Email<strong>[Email profissional]</strong></span></div><div><MapPin /><span>Morada<strong>{officeAddress}</strong></span></div><div><Clock3 /><span>Horário<strong>[Horário de atendimento]</strong></span></div></div></div><form className="contact-form" onSubmit={submitContact}><label>Nome<input name="name" type="text" placeholder="O seu nome" required /></label><label>Email<input name="email" type="email" placeholder="nome@empresa.com" required /></label><label>Telefone<input name="phone" type="tel" placeholder="+244 ..." /></label><label>Tipo de assunto<select name="subject" defaultValue="" required><option value="" disabled>Selecione uma opção</option><option>Consulta empresarial</option><option>Questão civil</option><option>Questão laboral</option><option>Outro assunto</option></select></label><label className="full-field">Mensagem<textarea name="message" rows={4} placeholder="Como podemos ajudar?" required /></label><button className="button button-navy" type="submit">Enviar Pedido de Consulta <ArrowUpRight /></button></form><div className="map-card"><iframe title="Localização do escritório MV-ADVOGADO em Av. Salvador Correia, Lobito" src={officeMap} loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div></div></section>
