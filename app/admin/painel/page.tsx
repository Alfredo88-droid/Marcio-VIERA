...
export default function AdminPanelPage() {
  ...
  async function loadLibrary() {
    const [{ data: files }, { data: rows }, { data: auth }] = await Promise.all([
      getSupabase().storage.from('site-library').list('photos', { sortBy: { column: 'created_at', order: 'desc' } }),
      getSupabase().from('library_articles').select('id,title,excerpt,content,created_at').order('created_at', { ascending: false }),
    ...
    if (files) setPhotos(files.filter((file) => file.name !== '.emptyFolderPlaceholder').map((file) => {
      ...
      return { name: file.name, path, url: getSupabase().storage.from('site-library').getPublicUrl(path).data.publicUrl }
    }))
  ...
  async function uploadPhotos(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      const { error } = await getSupabase().storage.from('site-library').upload(path, file, { upsert: false })
  ...
  async function saveArticle(event: React.FormEvent) {
    ...
    const result = editing ? await getSupabase().from('library_articles').update(payload).eq('id', editing.id) : await getSupabase().from('library_articles').insert(payload)
    if (result.error) setMessage(result.error.message); else { await loadLibrary(); closeModal() }
    setBusy(false)
  }

  async function deletePhoto(photo: Photo) { if (!confirm('Apagar esta foto?')) return; await getSupabase().storage.from('site-library').remove([photo.path]); loadLibrary() }
  async function deleteArticle(id: string) { if (!confirm('Apagar este artigo?')) return; await getSupabase().from('library_articles').delete().eq('id', id); loadLibrary() }
  ...
  return <main className="admin-panel-shell">
    <aside className="admin-sidebar"><Link href="/" className="admin-brand"><span>MV</span><b>ADVOGADO</b></Link><div className="admin-sidebar-label">GESTÃO DO SITE</div><nav className="admin-side-nav"><a className="is-active" href="#fotos">Fotos</a><a href="#artigos">Artigos</a><Link href="/#contactos">Ver site público <ExternalLink /></Link></nav><button className="admin-logout" onClick={() => getSupabase().auth.signOut()}><LogOut /> Sair</button></aside>
